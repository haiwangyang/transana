...
analyze_tissue_bias = function(strain, sex, tissue, level, alpha = 0.05){    
  ...
  part_samples$tissue = factor(part_samples$tissue, levels = c("wb", tissue))
...
for (strain in c("w1118", "oreR", species_in_final_order[2:8], "dgriG1")){
  ...
  for (tissue in tissues){
    for (sex in c("f", "m")){
      ...
      transana[[paste0(strain, "_", sex, "_", tissue)]] = x
    }
  }
}

save(transana,file=paste0(base_path, "output/transana_tissuebias.rtable"))
# load(paste0(base_path, "output/transana_tissuebias.rtable"))
```
