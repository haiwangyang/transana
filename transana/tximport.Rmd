...
gene2symbol = read.table(paste0(dir, species, ".FBtr2symbol"), header=TRUE)
...
for (strain in c("w1118", "oreR")){
  ...
  FBtrs_all = rownames(res)
  FBtrs_all_minus_m = FBtrs_all[! FBtrs_all %in% FBtrs_res_sig_male]
  FBtrs_unbiased = FBtrs_all_minus_m[! FBtrs_all_minus_m %in% FBtrs_res_sig_female]
  df_bias_m = data.frame("FBtr_ID"=as.character(unlist(FBtrs_res_sig_male)),"bias"="male")
  df_bias_unbias = data.frame("FBtr_ID"=FBtrs_unbiased,"bias"="unbiased")
  df_bias_f = data.frame("FBtr_ID"=as.character(unlist(FBtrs_res_sig_female)),"bias"="female")
  df_bias_total = rbind(df_bias_m,df_bias_unbias,df_bias_f)
  df_bias_total$FBtr_ID = as.character(df_bias_total$FBtr_ID)
  df_bias_total = df_bias_total[order(df_bias_total$FBtr_ID),]
  df_bias_total = df_bias_total[order(df_bias_total$FBtr_ID),]
